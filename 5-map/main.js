...
class MapCopy {
    ...
    set(key, value) {
        ...
        const bucketCell = this.bucket[Hash(key)];
    ...
    get(key) {
        ...
        return (_a = bucketCell.find((el) => el.key == key)) === null || _a === void 0 ? void 0 : _a.value;
    }
    has(key) {
        const hash = Hash(key);
        ...
        return this.bucket[hash].some((el) => el.key === key);
    }
    delete(key) {
        const hash = Hash(key);
        const BucketCell = this.bucket[hash];
